...
import { makeRequestApi, getUserByIdApi, getAllRoomchatApi, getAllUserApi, getRoomchatByIdApi } from "@/lib/api";
import { useSession } from "next-auth/react";
import { compareAsc, format } from "date-fns";
import { useAppDispatch, useAppSelector } from "@/app/redux/hooks";
import { AddListUser } from "@/app/redux/features/listUser/listUser.redux";
import { UserType } from "@/types/user";
import { AddListRoom, AddRoom, SearchRoom, SetRoomShow, UpdateMessage } from "@/app/redux/features/roomchat/roomchat.redux";
...
const ChatBox = () => {
  ...
  const [dataItems, setDataItems] = useState<{ [userId: string]: UserType }>({})
  const roomList = useAppSelector((state) => state.ListRoom.value)
  const tmpRoomList = useAppSelector((state) => state.ListRoom.tmpValue)
  const itemsShow = useAppSelector((state) => state.ListRoom.itemShow)
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const responseUser: UserType[] = await makeRequestApi(getAllUserApi, null, session?.refresh_token, session?.access_token);
        let newDataItems: { [userId: string]: UserType } = {};
        for (let i = 0; i < responseUser.length; i++) {
          newDataItems[responseUser[i].id ?? ""] = responseUser[i];
        }
        setDataItems(newDataItems)
        const responseRoom: RoomchatType[] = await makeRequestApi(getAllRoomchatApi, session?.userId, session?.refresh_token, session?.access_token);
        dispatch(AddListRoom(responseRoom))
        dispatch(SetRoomShow(responseRoom[0]))
      } catch (error) {
  ...
  useEffect(() => {
    ...
    roomList.forEach((data: RoomchatType) => {
      socketIo?.emit("join-room", { roomId: data.id });
    });
    socketIo?.on('new-user', async (user: { userId: string, roomId: string }) => {
      if (tmpRoomList.findIndex(it => it.id === user.roomId) == -1) {
        const responseUser: UserType = await makeRequestApi(getUserByIdApi, user.userId, session?.refresh_token, session?.access_token);
        if (responseUser) {
          setDataItems(pre => {
            let newData = { ...pre };
            newData[user?.userId ?? ""] = responseUser;
            return newData
          })
        }
        const responseRoom: RoomchatType = await makeRequestApi(getRoomchatByIdApi, user.roomId, session?.refresh_token, session?.access_token);
        if (responseRoom) {
          dispatch(AddRoom(responseRoom))
          socketIo?.emit("join-room", { roomId: user.roomId });
        }

      }
    })
    socketIo?.on('add-message', (msg: MessagesType) => {
  ...
  useEffect(() => {
    let dataSearch = Object.entries(dataItems)
      .filter(([userId, user]) => user.username?.toLowerCase()?.includes(searchBox))
      .map(([userId]) => userId);
  ...
  const onSubmitMessage = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    newMeg.roomId = itemsShow.id;
    socketIo?.emit("send-message", newMeg);
    setChatBox("");
  }
  const handleChangeRoom = (data: RoomchatType) => {
    dispatch(SetRoomShow(data))
  }
  ...
  const refChat: React.MutableRefObject<HTMLDivElement | null> = useChatScroll(itemsShow.messages)
      ...
      <div className="h-[calc(100vh-186px)] overflow-hidden sm:h-[calc(100vh-174px)]">
        <div className="h-full rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark xl:flex">
            ...
            <div className="flex max-h-full flex-col overflow-auto p-5">
              <div className="sticky mb-7">
                <input
                  ...
                  type="text"
                  className="w-full rounded border border-stroke bg-gray-2 py-2.5 pl-5 pr-10 text-sm outline-none focus:border-primary dark:border-strokedark dark:bg-boxdark-2" placeholder="Search..." typeof="text"
                />
                <div className="absolute right-4 top-1/2 -translate-y-1/2">
                  <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M8.25 3C5.3505 3 3 5.3505 3 8.25C3 11.1495 5.3505 13.5 8.25 13.5C11.1495 13.5 13.5 11.1495 13.5 8.25C13.5 5.3505 11.1495 3 8.25 3ZM1.5 8.25C1.5 4.52208 4.52208 1.5 8.25 1.5C11.9779 1.5 15 4.52208 15 8.25C15 11.9779 11.9779 15 8.25 15C4.52208 15 1.5 11.9779 1.5 8.25Z" fill="#637381"></path><path fill-rule="evenodd" clip-rule="evenodd" d="M11.957 11.958C12.2499 11.6651 12.7247 11.6651 13.0176 11.958L16.2801 15.2205C16.573 15.5133 16.573 15.9882 16.2801 16.2811C15.9872 16.574 15.5124 16.574 15.2195 16.2811L11.957 13.0186C11.6641 12.7257 11.6641 12.2508 11.957 11.958Z" fill="#637381"></path></svg>
                </div>
              </div>

              {roomList.map((item, index) => {
                return (
                  <div key={item.id} className="no-scrollbar max-h-full space-y-2.5 overflow-auto">
                    <div
                      onClick={() => handleChangeRoom(item)}
                      className="flex cursor-pointer items-center rounded px-4 py-2 hover:bg-gray-200 dark:hover:bg-strokedark"
                    >
                      <div className="relative mr-3.5 h-11 w-full max-w-11 rounded-full">
                        <Image
                          width={56}
                          height={56}
                          src={dataItems[item.userId ?? ""]?.imgDisplay ?? ""}
                          alt="User"
                      ...
                      <div className="w-full">
                        <h5 className="text-sm font-medium text-black dark:text-white">
                          {dataItems[item.userId ?? ""]?.username ?? ""}
                        </h5>
                        {item.messages.length > 0 && (
                          <p className="text-sm">{item.messages[item.messages.length - 1].content}</p>
                        )}
                      </div>
                    </div>
                  </div>
          ...
          <div className="flex h-full flex-col border-l border-stroke dark:border-strokedark xl:w-3/4">
            <div className="sticky flex items-center justify-between border-b border-stroke px-6 py-4.5 dark:border-strokedark">
              <div className="flex items-center">
                <div className="mr-4.5 h-13 w-full max-w-13 overflow-hidden rounded-full">
                  <Image
                    width={52}
                    height={52}
                    src={dataItems[itemsShow.userId ?? ""]?.imgDisplay ?? ""}
                    className="rounded-full"
                    alt="User"
                    style={{
                      width: "auto",
                ...
                <div>
                  <h5 className="font-medium text-black dark:text-white">
                    {dataItems[itemsShow.userId ?? ""]?.username}
                  </h5>
              ...
              <div>
                <div className="dropdown dropdown-end">
                  <button tabIndex={0} role="button" type="button" className="text-[#98A6AD] hover:text-body">
                    <svg aria-hidden="true" className="fill-current" width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M2.25 11.25C3.49264 11.25 4.5 10.2426 4.5 9C4.5 7.75736 3.49264 6.75 2.25 6.75C1.00736 6.75 0 7.75736 0 9C0 10.2426 1.00736 11.25 2.25 11.25Z" fill=""></path><path d="M9 11.25C10.2426 11.25 11.25 10.2426 11.25 9C11.25 7.75736 10.2426 6.75 9 6.75C7.75736 6.75 6.75 7.75736 6.75 9C6.75 10.2426 7.75736 11.25 9 11.25Z" fill=""></path><path d="M15.75 11.25C16.9926 11.25 18 10.2426 18 9C18 7.75736 16.9926 6.75 15.75 6.75C14.5074 6.75 13.5 7.75736 13.5 9C13.5 10.2426 14.5074 11.25 15.75 11.25Z" fill=""></path></svg>
                  </button>
                  <div tabIndex={0} className="dropdown-content menu first-line:absolute right-0 top-full z-40 w-40 space-y-1 rounded-sm border border-stroke bg-white p-1.5 shadow-default dark:border-strokedark dark:bg-boxdark">
            ...
            <div ref={refChat} className="no-scrollbar max-h-full h-full space-y-3.5 overflow-auto px-6 py-7.5">

              {itemsShow.messages.map((item, index) => {
                if (item.userId === session?.userId) {
                  // Nếu là tôi
                  return (
                    <div key={index} className="ml-auto max-w-100">
                      <div className="mb-2.5 rounded-2xl rounded-br-none bg-primary px-5 py-3 text-pretty">
                        {item.urlFile && item.urlFile[0] !== undefined && (
                          ...
                          <Image
                            ...
                            onDoubleClick={() => {
                              ...
                              if (item && item.urlFile && item.urlFile.length > 0) {
                                myWindow?.document.write(`<img src="${item.urlFile[0]}"/>`);
                              } else {
                                // Handle the case when item.urlFile is undefined or empty
                                myWindow?.document.write("Image not available");
                              }
                            }}
                          />
                        )}
                      ...
                      <p className="text-right text-xs">{format(new Date(item.updateAt ?? ""), "yyyy-MM-dd : hh:mm a")}</p>
                    </div>
                  );
                } else {
                  // Nếu là khách hàng
                  return (
                    <div key={index} className="max-w-100">
                      <p className="mb-2.5 text-sm font-medium">{dataItems[item.userId ?? ""]?.username}</p>
                      <div className="mb-2.5 rounded-2xl rounded-tl-none bg-gray px-5 py-3 dark:bg-boxdark-2 text-pretty">
                        {item.urlFile && item.urlFile[0] !== undefined && (
                      ...
                      <p className="text-xs">{format(new Date(item?.updateAt ?? ""), "yyyy-MM-dd : hh:mm a")}</p>
                    </div>
                  );
                }
              })}
            ...
            <div className="sticky bottom-0 border-t border-stroke bg-white px-6 py-5 dark:border-strokedark dark:bg-boxdark">
              <form onSubmit={onSubmitMessage} action="" className="flex items-center justify-between space-x-4.5">
                <div className="relative w-full">
                  <input value={chatBox} onChange={(e) => setChatBox(e.target.value)} type="text" placeholder="Type something here" className="h-13 w-full rounded-md border border-stroke bg-gray pl-5 pr-19 text-black placeholder-body outline-none focus:border-primary dark:border-strokedark dark:bg-boxdark-2 dark:text-white" />
                  <div className="absolute right-5 top-1/2 inline-flex -translate-y-1/2 items-center justify-end space-x-4">
                    <input onChange={handleImageChange} placeholder="uploadFile" id="upload-photo" type="file" name="photo" className="z-[-1] opacity-0 absolute w-0 h-0" />
                    <label htmlFor="upload-photo" className="hover:text-primary cursor-pointer">
                ...
                <button className="flex h-13 w-full max-w-13 items-center justify-center rounded-md bg-primary text-white hover:bg-opacity-90">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M22 2L11 13" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path><path d="M22 2L15 22L11 13L2 9L22 2Z" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path></svg>
                </button>
              </form>
            </div>
          </div>
