...
export const ListRoom = createSlice({
    ...
    reducers: {
        ...
        SetRoomShow: (state, action: PayloadAction<string>) => {
            let index = state.tmpValue.findIndex(it => it.id === action.payload)
            if (index !== -1) {
                state.itemShow == state.tmpValue[index]
            }
            console.log(state.tmpValue[index])
        }
    }
})
