...
const SignUp: React.FC = (props: any) => {
  ...
  const handleOtp = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (rePasswordUser == "") return;
    if (userName == "") return;
    if (passwordUser != rePasswordUser) return;
  ...
  const handleSignUp = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!res?.error) {
      router.push(props.searchParams.callbackUrl ?? "http://localhost:3000/dashboard");
    }
  }
