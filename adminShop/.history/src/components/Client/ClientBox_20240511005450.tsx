    ...
    <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
      ...
      <dialog id="my_modal_view" className="modal modal-bottom sm:modal-middle">
        <div className="modal-box">
          ...
          <p className="pb-1">Date: {new Date(itemsShow?.birthday ?? "").toDateString()}</p>
          <Image
      ...
      <dialog id="my_modal_edit" className="modal modal-bottom sm:modal-middle">
              ...
              onChange={e => {
                setItemsShow(prevItems => {
                  if (e.target.checked) {
                    ...
                    newItems.role.push("WEREHOUSEMANGER");
                    return newItems;
                  } else {
                    if (prevItems?.role) {
                      const filteredRole = prevItems.role.filter(role => role !== "WEREHOUSEMANGER");
                      return { ...prevItems, role: filteredRole };
                    }
                  }
                  return prevItems;
                });
              ...
              onChange={e => {
                setItemsShow(prevItems => {
                  if (e.target.checked) {
                    ...
                    newItems.role.push("TELESALES");
                    return newItems;
                  } else {
                    if (prevItems?.role) {
                      const filteredRole = prevItems.role.filter(role => role !== "TELESALES");
                      return { ...prevItems, role: filteredRole };
                    }
                  }
                  return prevItems;
        ...
        <div className="form-control w-65">
          <label className="cursor-pointer label">
            <span className="label-text">BANED</span> 
            <input 
onChange={e => {
  setItemsShow(prevItems => {
    if (prevItems) {
      const newItems = { ...prevItems };
      if (e.target.checked) {
        // Add "BANNED" to the role array if it doesn't already exist
        if (!newItems.role || !newItems.role.includes("BANNED")) {
          newItems.role = [...(newItems.role || []), "BANNED"];
        }
      } else {
        // Remove "BANNED" from the role array if it exists
        newItems.role = (newItems.role || []).filter(role => role !== "BANNED");
      }
      return newItems;
    }
    return null; // Return null if prevItems is null
  });
}}
              type="checkbox" 
              className="toggle toggle-accent" 
              checked={itemsShow?.role?.includes("BANED")} 
            />
          </label>
        </div>
      </div>
