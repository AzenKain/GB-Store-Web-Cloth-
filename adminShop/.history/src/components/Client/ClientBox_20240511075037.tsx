...
const ClientBox = () => {
  ...
  const handleSubmit = async (role: string[]) => {
    ...
    const responseData: UserType = await makeRequestApi(updateRoleUserApi, role, session?.refresh_token, session?.access_token);
    if (responseData) {
      ...
      <dialog id="my_modal_edit" className="modal modal-bottom sm:modal-middle">
        <div className="modal-box">
          ...
          <div className="flex flex-col">
            ...
            <button 
              onClick={async () => await handleSubmit(itemsShow?.role ?? ["USER"])}
