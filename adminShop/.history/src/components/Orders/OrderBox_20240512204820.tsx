...
const SalesBox = () => {
    ...
    const [userShow, setUserShow] = useState<UserType| null>(null)
    const elementSelect = {
    ...
    const handleShow = (items: OrderType, model: string) => {
        ...
        let indexUser = userList.findIndex(it => it.id === items.userId)
        if (indexUser == -1) return;
        setUserShow(userList[indexUser])
        if (modal) {
            modal.showModal();
        }
    }
        ...
        <div className="flex flex-col gap-10 mt-2">
            ...
            <dialog id="my_modal_view" className="modal">
                <div className="modal-box w-11/12 max-w-5xl">
                    ...
                    <div className="mt-4">
                        <InvoiceCard itemsShow={itemsShow} dataItems={dataItems} userShow={userShow}/>
                    </div>
                </div>
            </dialog>
        </div>
