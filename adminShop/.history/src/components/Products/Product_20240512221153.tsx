        ...
        {productList.map((product, key) => (
          ...
          >
            ...
            <div className="col-span-2 flex items-center gap-1">
              ...
              <button aria-label="Submit" onClick={() => handleShow(-1, "my_modal_control", "VIEW")} className="btn btn-info">
            ...
            <dialog id="my_modal_view" className="modal">
                <div className="modal-box w-11/12 max-w-5xl">
                    ...
                    <div className="mt-4">
                    <div>
                    <h2>{productList[productSelect].productName}</h2>
                    <p>Cost: {productList[productSelect].cost}</p>
                    <p>Price: {productList[productSelect].price}</p>
                    <p>Stock Quantity: {productList[productSelect].stockQuantity}</p>
                    <p>Description: {productList[productSelect].description}</p>
                    <h3>Product Images:</h3>
                    <div>
                      {productList[productSelect].imgDisplay.map((image, index) => (
                        <img key={index} src={image.url} alt={`Product Image ${index + 1}`} style={{ maxWidth: '200px' }} />
                      ))}
                    </div>
                    <h3>Product Details:</h3>
                    <ul>
                      <li>Tags: {productList[productSelect].detail?.tags.join(', ')}</li>
                      <li>Company: {productList[productSelect].detail?.company}</li>
                      <li>Materials: {productList[productSelect].detail?.materials.join(', ')}</li>
                    </ul>
                    {productList[productSelect].sales && (
                      <div>
                        <h3>Sales Information:</h3>
                        <p>On Sale: {productList[productSelect].sales.isSales ? 'Yes' : 'No'}</p>
                        <p>Discount Percentage: {productList[productSelect].sales.percents}%</p>
                        <p>Sale End Date: {productList[productSelect].sales.end.toLocaleDateString()}</p>
                      </div>
                    )}
                    {/* You can add more sections for other product-related information */}
                  </div>
                    </div>
                </div>
            </dialog>
