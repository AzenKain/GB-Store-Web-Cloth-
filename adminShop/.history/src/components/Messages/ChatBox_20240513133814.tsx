...
import { AddListRoom, SetRoomShow } from "@/app/redux/features/roomchat/roomchat.redux";
...
const ChatBox = () => {
  const [dataRoomChat, setDataRoomChat] = useState<RoomchatType[]>([])
  const [dataChat, setDataChat] = useState<MessagesType[]>([])
  const [chatBox, setChatBox] = useState<string>("")
  const [boxCurrent, setBoxCurrent] = useState<number>(0);
  ...
  useEffect(() => {
    ...
    dataRoomChat.forEach((data: RoomchatType) => {
  ...
  const onSubmitMessage = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    newMeg.roomId = dataRoomChat[boxCurrent].id;
