...
const ChatBox = () => {
  ...
  useEffect(() => {
    ...
    socketIo?.on('add-message', async (msg: MessagesType) => {
      if (tmpRoomList.findIndex(it => it.id === msg.roomId) == -1) {
        const responseUser: UserType = await makeRequestApi(getUserByIdApi, msg.userId, session?.refresh_token, session?.access_token);
        if (responseUser) {
          setDataItems(pre => {
            let newData = {...pre};
            newData[msg?.userId ?? ""] = responseUser;
            return newData
          })
        }
        const responseRoom: RoomchatType = await makeRequestApi(getRoomchatByIdApi, msg.roomId, session?.refresh_token, session?.access_token);
        if (responseRoom) {
          dispatch(AddRoom(responseRoom))
        }
        socketIo?.emit("join-room", { roomId: msg.roomId });
        return;
      }
      else {
