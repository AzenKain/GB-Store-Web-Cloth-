...
import { makeRequestApi, getUserByIdApi, getAllRoomchatApi, getAllUserApi } from "@/lib/api";
  ...
  useEffect(() => {
    ...
    socketIo?.on('add-message', async (msg: MessagesType) => {
      if (tmpRoomList.findIndex(it => it.id === msg.roomId) == -1) {
        const responseRoom: RoomchatType = await makeRequestApi(getRoomchat, session?.userId, session?.refresh_token, session?.access_token);
        return;
      }
