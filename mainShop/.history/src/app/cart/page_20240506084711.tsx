...
import { getCartDetailApi, getProductByIdApi, makeRequestApi, updateCartApi } from "@/lib/api";
import { useEffect, useState } from "react";
import { CartItemType, CartType } from "@/types/cart";
import { AddDataCart, UpdateProductCart } from "../redux/features/cart/cart.redux";
...
export default function Cart() {
    ...
    const handleUpdateCart = async (product: CartItemType) => {
        if (session !== null) {
                    ...
                    {dataCart.items.map((product, index) => (
                        <div key={index} className="flex items-start max-sm:flex-col gap-8 py-6">
                            <div className="h-52 shrink-0">
                                <Image
                                    src={dataItems[product.productId]?.imgDisplay.find(img => img?.link?.includes(product.color.toLowerCase()))}
                                    width={100}
                                    height={100}
                ...
                <div className="shadow-md p-6">
                    ...
                    <ul className="text-[#333] divide-y mt-6">
                        ...
                        <li className="flex flex-wrap gap-4 text-md py-4 font-bold">
                            Total <span className="ml-auto">{dataCart.totalPrice * 1.1 + shippingCost}đ</span>
                        </li>
                    </ul>
                    <button
