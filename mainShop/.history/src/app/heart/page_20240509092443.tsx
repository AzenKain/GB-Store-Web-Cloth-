...
import { addHeartApi, getProductByIdApi, getUserDetailApi, makeRequestApi, removeHeartApi } from "@/lib/api"
        ...
        <div className="font-[sans-serif] mx-auto mt-20">
            <div className="p-4 mx-auto lg:max-w-7xl sm:max-w-full">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    ...
                    {user.heart.map((product, index) => (
                        <div key={index} className="bg-white rounded-2xl p-6 hover:-translate-y-2 transition-all relative">
                            <button
                                onClick={async () => await handleUpdateHeart(dataItems[product]?.id ?? "", !user?.heart.includes(dataItems[product]?.id ?? ""))}
                            ...
                            >
                                {user?.heart.includes(dataItems[product]?.id ?? "") ? (
                            ...
                            <div className="w-11/12 h-[220px] overflow-hidden mx-auto aspect-w-16 aspect-h-8 md:mb-2 mb-4">
                                <Image
                                    src={dataItems[product]?.imgDisplay[0].url}
                                    alt={dataItems[product]?.productName}
                            ...
                            <div>
                                <Link href={`/product/${dataItems[product]?.id}`} className="text-lg font-bold text-gray-800">
                                    {dataItems[product]?.productName}
                                </Link>
                                <p className="text-gray-500 text-sm mt-2">
                                    {dataItems[product]?.detail?.company}
                                </p>
                                <h4 className="text-lg text-gray-700 font-bold mt-4">{dataItems[product]?.price}đ</h4>
                            </div>
                            <div className="flex mt-[5px] flex-row justify-center gap-1 justify-self-center">
                                {dataItems[product]?.color.map((value: string, index) => (
                                    <div
                                        key={index}
