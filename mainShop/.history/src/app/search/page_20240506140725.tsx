                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                    ...
                                    {products.map((product, index) => (
                                        <div key={product.id} className="bg-white rounded-2xl p-6 hover:-translate-y-2 transition-all relative">
                                            <button 
                                                onClick={async () => handleUpdateHeart(product?.id ?? "", user.heart.includes(product?.id ?? ""))}
                                                className="bg-gray-100 w-10 h-10  hover:w-12 hover:h-12 flex items-center justify-center rounded-full cursor-pointer absolute top-4 right-4"
                                            >
