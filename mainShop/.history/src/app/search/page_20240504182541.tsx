...
import { AddListProduct } from '../redux/features/product/product.redux';
...
export default function Search() {
   ...
   useEffect(() => {
        // Logic to apply filters to products
        const filteredProducts = products.filter(product => {
            // Check if product meets all filter criteria
            return filters.every(filter => {
                // Check if the product's filter option is checked
                return filter.options.some(option => option.checked && option.value === product.filterValue);
            });
        });
        // Update state with filtered products
        // (You might want to store filtered products in state or redux store for better performance)
                                                                    ...
                                                                    <div key={option.value} className="flex items-center">
                                                                        <input
                                                                            ...
                                                                            onChange={() => {
                                                                                ...
                                                                                dispatch(updateFilter({ id: section.id, value: option.value, checked: !option.checked }))
                                                                            }}
                            ...
                            <div className="lg:col-span-3">
                                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                    ...
                                    {products.map((product, index) => (
                                        <div key={product.id} className="bg-white rounded-2xl p-6 cursor-pointer hover:-translate-y-2 transition-all relative">
