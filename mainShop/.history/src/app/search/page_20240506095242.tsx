...
import { AddFilterProduct, AddListProduct } from '../redux/features/product/product.redux';
import { getAllProductApi } from '@/lib/api';
...
const banner = {
    'men': './image/cover/men.jpg',
    'women': './image/cover/women.jpg',
    'Accessories': '/image/cover/phu-kien.jpg',
    'hot-sales': './image/cover/best-seller.jpg',
    'new-arrival': './image/cover/new-arrivals.jpg'
}
...
export default function Search(prop: Prop) {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            for (let param in prop.searchParams) {
                if (typeof prop.searchParams[param] === "string") {
                    dispatch(updateFilter({ id: param, value: prop.searchParams[param], checked: true }));
                    continue;
                }
            
                // Nếu prop.searchParams[param] là một mảng chuỗi, chuyển đổi thành chuỗi trước khi xử lý
                const paramValueArray: string[] = prop.searchParams[param] as string[];
                const paramValueString: string = paramValueArray.join(', '); // Hoặc bất kỳ cách chuyển đổi mảng thành chuỗi phù hợp khác
            
                dispatch(updateFilter({ id: param, value: paramValueString, checked: true }));
            
                for (let value of paramValueArray) {
                    dispatch(updateFilter({ id: param, value, checked: true }));
                }
            
                if (prop.searchParams[param] in banner) {
                    setBannerDisplay(banner[prop.searchParams[param]]);
                }
            }
        };
        fetchData();
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                    ...
                                    {products.map((product, index) => (
                                        <div key={product.id} className="bg-white rounded-2xl p-6 hover:-translate-y-2 transition-all relative">
                                            <div className="bg-gray-100 w-10 h-10  hover:w-12 hover:h-12 flex items-center justify-center rounded-full cursor-pointer absolute top-4 right-4">
