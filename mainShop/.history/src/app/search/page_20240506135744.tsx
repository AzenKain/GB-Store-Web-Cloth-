...
import { addHeartApi, getAllProductApi, makeRequestApi } from '@/lib/api';
import { ProductType } from '@/types/product';
import Link from 'next/link';
import Image from 'next/image';
import { useSearchParams } from 'next/navigation'
import { FilterSection } from '@/types/filter';
import { AddHeart, UpdateUser } from '../redux/features/user/user.redux';
...
export default function Search() {
    ...
    const handleUpdateHeart = async (productId: string, value: boolean)  => {
        if (value) {
            if (session !== null) {
                let dataUser: UserType = await makeRequestApi(addHeartApi, null, session?.refresh_token, session?.access_token)
                if (dataUser !== null) {
