...
export default function Search() {
    ...
    useEffect(() => {
        ...
        fetchData();
    }, [dispatch, searchParams, filters, banner]);

    useEffect(() => {
        dispatch(AddFilterProduct(filters));
        // Lưu trạng thái filter vào localStorage
        localStorage.setItem('filters', JSON.stringify(filters));
    }, [filters, dispatch]);

    // Khi component được render, kiểm tra xem có trạng thái filter đã lưu trong localStorage không
    useEffect(() => {
        const storedFilters = JSON.parse(localStorage.getItem('filters'));
        if (storedFilters) {
            dispatch(AddFilterProduct(storedFilters));
        }
    }, [dispatch]);
