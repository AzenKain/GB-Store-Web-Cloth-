    ...
    reducers: {
        ...
        AddProduct: (state, action: PayloadAction<CartItemType>) => {
            ...
            if (indexValue !== -1) {
                ...
                let newTotalPrice = initialState.value.items.map(item => Ơitem.price * it)
                return {
                ...
                let newTotalPrice = initialState.value.totalPrice += action.payload.price * action.payload.quantity
                return {
                    items: newProductList,
                    totalPrice: newTotalPrice
                }
