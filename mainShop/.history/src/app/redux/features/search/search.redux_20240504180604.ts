...
export const CartRedux = createSlice({
    ...
    reducers: {
        updateFilter: (state, action: PayloadAction<FilterUpdate>) => {
            ...
            if (indexValue == -1) return initialState;
            let indexOption = initialState.value[indexValue].options.findIndex(
                it => it.value === action.payload.value
            )
            if (indexOption == -1) return initialState;
            let newFilter = [...initialState.value];
            newFilter[indexValue].options[indexOption].checked = action.payload.checked;
            return {
                value: newFilter
            }
        },
        resetFilter: (state, action: PayloadAction<FilterUpdate>) => {
            return {
                value: initialState
            }
        }
...
export const { updateFilter, resetFilter } = CartRedux.actions;
