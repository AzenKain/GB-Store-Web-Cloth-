...
import { createVnpayApi, getOrderByIdApi, getProductByIdApi, makeRequestApi } from "@/lib/api";
...
export default function Order({ params }: { params: { orderId: string } }) {
    ...
    const handlePaynow = async () => {
        let dataReturn2 = await makeRequestApi(createVnpayApi, dataReturn2.id, session?.refresh_token, session?.access_token)
        if (!dataReturn2) {
                ...
                <div className="main-box border border-gray-200 rounded-xl pt-6 max-w-xl max-lg:mx-auto lg:max-w-full">
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between px-6 pb-6 border-b border-gray-200">
                        ...
                        {orderData.isPay?.isPaid == false && orderData.paymentMethods == "VNPAY" && (
                            <div className="cursor-pointer text-center text-3xl font-bold border-4 bg-green-300 px-4 py-2 rounded-full">
                                Pay Now
                            </div>
                        )}
