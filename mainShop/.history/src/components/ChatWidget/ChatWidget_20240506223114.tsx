...
export default function ChatWidget() {
    ...
    const [messages, setMessages] = useState<any | null>(intialMesages);
    ...
    const refChat: React.MutableRefObject<HTMLDivElement | null> = useChatScroll(messages);

    if (session == null) return null;

    return (
        <div className="dropdown dropdown-top dropdown-end">
            <div tabIndex={0} role="button" className="m-1 btn btn-secondary">
                        ...
                        <div ref={refChat} className="no-scrollbar w-full overflow-auto space-y-6 py-8 text-base leading-7 text-gray-600 h-[400px] overflow-y-auto">
                            <ul className="space-y-4 w-full px-4">
                                {messages.map((item : any, idx : any) => (
                                    <li
                                        key={idx}
                                        className={`flex justify-start ${item.role === "user" ? "ml-20 justify-end" : "mr-20"
                                            }`}
                                    >
