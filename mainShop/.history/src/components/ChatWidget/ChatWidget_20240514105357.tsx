...
import { UpdateMessage, UpdateRoomchat } from "@/app/redux/features/roomchat/roomchat.redux";

type MySocket = Socket<DefaultEventsMap, DefaultEventsMap>

export default function ChatWidget() {
    const { data: session } = useSession()
    const dataRoomChat = useAppSelector((state) => state.RoomchatRedux.value)
    ...
    useEffect(() => {
        ...
        socket.emit("join-room", { roomId: dataRoomChat?.id });
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                dispatch(UpdateRoomchat(responseData))
    ...
    const refChat: React.MutableRefObject<HTMLDivElement | null> = useChatScroll(dataRoomChat?.messages);
                    ...
                    <div className="divide-y divide-gray-300/50 border-t w-full border-gray-300/50">
                        <div ref={refChat} className="no-scrollbar w-full overflow-auto space-y-6 py-8 text-base leading-7 text-gray-600 h-[400px] overflow-y-auto">
                            <ul className="space-y-4 w-full px-4">
                                {dataRoomChat?.messages.map((item, idx) => (
                                    <li
                                        key={item.id}
                                    ...
                                    >
                                        ...
                                        {item.content != '' && (<p className="bg-gray-100 p-2 rounded-md">{item.content}</p>)}
                        ...
                        >
                            ...
                            <input
                                ...
                                accept="image/*"
                                name="photo"
                                className="z-[-1] opacity-0 absolute w-0 h-0"
                            />
