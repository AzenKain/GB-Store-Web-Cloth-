...
import { AddFilterProduct, AddListProduct, FindByName, SortProduct } from '../redux/features/product/product.redux';
...
export default function Search() {
    ...
    const [sortValue, setSortValue] = useState<string>("Most Popular")
    ...
    useEffect(() => {
        ...
        dispatch(SortProduct(sortValue))
    }, [filters, search, sortValue, dispatch])
                                        ...
                                        <div className="py-1">
                                            {sortOptions.map((option) => (
                                                <Menu.Item key={option.name}>
                                                    {({ active }) => (
                                                        <a
                                                            onClick={() => setSortValue(option.name)}
                                                            className={classNames(
                                                                option.name == sortValue ? 'font-medium text-gray-900' : 'text-gray-500',
                                                                active ? 'bg-gray-100' : '',
